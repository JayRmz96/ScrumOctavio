...
export class ServicioSudadera {
  ...
  tomarJsonSlide() {
    ...
    return this._http.get(this.url + 'mostrar-sudaderas').pipe(map(resultado => resultado.json()));
  }

  subirImagenSudadera(url, items, token, imagen) {
    ...
    if (!imagen) {
      ...
      return new Promise(function(resolver, rechazar) {
        ...
        formData.append('talla', items.talla);
  ...
  actualizarItemSudadera(url, items, token, imagen) {
  ...
  borrarItemSlide(id) {
    ...
    return this._http.delete(this.url + 'borrar-sudaderas/' + id, {headers: headers}).pipe(map(resultado => resultado.json()));
