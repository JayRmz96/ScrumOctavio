...
export class ServicioSticker {
  ...
  tomarJsonSlide() {
    ...
    return this._http.get(this.url + 'mostrar-stickers').pipe(map(resultado => resultado.json()));
  }

  subirImagenSticker(url, items, token, imagen) {
  ...
  actualizarItemSticker(url, items, token, imagen) {
    ...
    if (!imagen) {
      ...
      return new Promise(function(resolver, rechazar) {
        ...
        formData.append('nombre', items.nombre);
        formData.append('size', items.sexo);
        formData.append('color', items.color);
        formData.append('precio', items.precio);
        formData.append('descripcion', items.descripcion);
  ...
  borrarItemSlide(id) {
    ...
    return this._http.delete(this.url + 'borrar-stickers/' + id, {headers: headers}).pipe(map(resultado => resultado.json()));
